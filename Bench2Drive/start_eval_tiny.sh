...
$PYTHON $PROGRAM \
    ...
    --repetitions=1 \
    --track=SENSORS \
    --checkpoint=/workspace/end2end_AD_DriveTransformer/Bench2Drive/DriveTransformer_b2d_only_traj/eval_bench2drive_dev_4.json \
    --agent=/workspace/end2end_AD_DriveTransformer/Bench2Drive/leaderboard/team_code/drivetransformer_vis_agent.py \
    --agent-config=/workspace/end2end_AD_DriveTransformer/DriveTransformer/adzoo/drivetransformer/configs/drivetransformer/drivetransformer_tiny.py+/workspace/end2end_AD_DriveTransformer/DriveTransformer/ckpts/iter_93750.pth \
    --debug=0 \
    --record="" \
    --port=30001 \
